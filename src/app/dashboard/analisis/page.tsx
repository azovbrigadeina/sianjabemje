...
export default function AnalisisPage() {
  ...
  const loadTree = useCallback(async () => {
    // 1. Optimistic Cache Hydration: langsung render seketika jika cache lokal tersedia
    try {
      ...
      if (cachedBulk && cachedBulk.unitKerja && cachedBulk.jabatan) {
        ...
        const { roots, expandState } = buildTreeNodes(cOpds, cJabatans, cAnjabSet);
        setTreeData(roots);
        setExpandedNodes(prev => Object.keys(prev).length > 0 ? prev : expandState);
        setIsLoadingTree(false); // Langsung render 0 ms!
      } else {
    ...
    // 2. Network Fetch Ringan (hanya 3 tabel + status summary ringkas)
    try {
      ...
      const { roots, expandState } = buildTreeNodes(opds, jabatans, anjabSet);
      setTreeData(roots);
      setExpandedNodes(prev => Object.keys(prev).length > 0 ? prev : expandState);
    } catch (err) {
      console.error("Gagal memuat pohon anjab:", err);
    } finally {
