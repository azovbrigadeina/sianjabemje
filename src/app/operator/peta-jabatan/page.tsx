...
export default function OperatorPetaJabatanPage() {
  const { user } = useUser();
  ...
  // Fetch full data for Operator's specific OPD
  useEffect(() => {
    const opdId = user?.unitKerjaId;
    if (!opdId) return;
    ...
    const loadOpdHierarchy = async () => {
      ...
      try {
        ...
        // Find operator's own OPD name
        const myOpd = opdList.find(o => o.id === opdId);
        if (myOpd) {
          setOpdName(myOpd.nama);
        }

        // Get descendant sub-unit IDs (so they can see their sub-units if any)
        const targetUnitIds = getDescendantUnitIds(opdId, opdList);
        
        // Filter positions belonging to their OPD structure
    ...
    loadOpdHierarchy();
  }, [user, getDescendantUnitIds, buildHierarchyTree]);
    ...
    <div className={styles.container}>
      ...
      <div className="glass-panel">
        ...
        <div className={styles.toolbar}>
          <div className={styles.toolbarLeft} style={{ minWidth: 'auto', flex: 1 }}>
            <span className={styles.opdSelectLabel} style={{ fontSize: '1rem', color: 'hsl(var(--primary))' }}>
              🏢 {opdName}
            </span>
          </div>
          ...
          <div className={styles.toolbarRight}>
            ...
            <button
              ...
              title="Jika diaktifkan, cetakan A4 akan dipisah per-Bidang secara otomatis agar muat rapi."
            >
        ...
        >
          {isLoading ? (
            ...
            <div className={styles.emptyContainer}>
              ...
              <p>OPD Anda belum memiliki data struktur jabatan terdaftar.</p>
            </div>
          ) : (
            <div 
      ...
      {/* PRINT-ONLY PAGES CONTAINER */}
      <div className={styles.printPagesContainer}>
        {splitPrintPages && hierarchy.length > 0 ? (
          <>
            {/* Sheet 1: Peta Induk (Summary structural map) */}
            {hierarchy.map(rootNode => {
              const majorBranches = rootNode.children;
              return (
                <div key={`print-op-summary-${rootNode.id}`} className={styles.printPage}>
            ...
            {hierarchy.map(rootNode => {
              ...
              return majorBranches.map((branch, idx) => (
                <div key={`print-op-detail-${branch.id}`} className={styles.printPage}>
                  <div className={styles.printPageHeader}>
          ...
          hierarchy.map(rootNode => (
            <div key={`print-op-full-${rootNode.id}`} className={styles.printPage}>
              <div className={styles.printPageHeader}>
